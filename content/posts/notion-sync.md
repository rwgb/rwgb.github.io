...
|----------|------|---------|
| **Name** | Title | Your post title (this is default) |
| **Status** | Select | Draft, Ready, Published |
| **Published Date** | Date | When to publish |
| **Slug** | Text | URL-friendly version (my-post-title) |
| **Description** | Text | SEO meta description |
| **Tags** | Multi-select | Topics and keywords |
...
2. **Can be triggered manually** for immediate syncs
3. **Pulls your code** including the Hugo theme
4. **Installs Python** and dependencies
5. **Runs the sync script** with your Notion credentials
6. **Commits changes** if new posts were added
7. **Triggers Hugo rebuild** automatically (your existing workflow)
...
2. **Look at your repository** for new files in `content/posts/`
3. **Wait 2-3 minutes** for Hugo to rebuild
4. **Visit your site** and see your new post!
...
2. Regenerate token in Notion if needed
3. Update the secret
...
2. **Download images to repo** (modify script to download)
3. **Accept temporary URLs** (good enough for most use cases)
...
### On Mobile:
1. Open Notion app
2. Create new entry in database
3. Write using voice-to-text while walking your dog
4. Hit "Published"
5. Post is live within an hour

### For Scheduled Posts:
1. Write whenever you want
2. Set Status to "Ready"
3. Use Notion's reminder feature for publish date
...
Add a repository dispatch webhook to trigger syncs from Notion automations (if you're on Notion Enterprise).

### Multiple Databases

Want separate databases for blog posts and projects?

1. Create second database
2. Add `NOTION_PROJECTS_DATABASE_ID` secret
3. Modify script to sync both
4. Output projects to `content/projects/`

### Draft Preview

Want to preview drafts locally?

```bash
# Temporarily change the filter
# In notion-sync.py, change 'Published' to 'Ready'
python scripts/notion-sync.py
hugo server -D
```
...
1. Download Notion images
2. Compress them
3. Save to `static/img/`
...
- ✅ You write on mobile sometimes
- ✅ You like Notion's editor
- ✅ You want database views of content
- ✅ You collaborate with others

**Stick with direct Hugo if:**
- ❌ You rarely post (monthly or less)
- ❌ You only write at your desk
- ❌ You prefer markdown editors
- ❌ You don't need Notion's features

For me? Game changer. I went from "I should write a post" to actually publishing 3x more content. The reduced friction matters.

## Wrapping Up

You now have a professional CMS powering your static site. Write in Notion, publish automatically, and your site updates itself. No servers to maintain, no databases to manage, no hosting fees.

The best part? This scales. Whether you publish once a month or once a day, the system handles it. Focus on writing great content—let the robots handle the rest.
...
- [My sync script on GitHub](https://github.com/rwgb/rwgb.github.io)

---

**Questions?** Drop a comment below or reach out on [GitHub](https://github.com/rwgb). If this helped you, consider sharing it with other Hugo users!

**Update**: Since publishing this, several readers have asked about syncing the other direction (GitHub → Notion). That's definitely possible—let me know if you want a follow-up article on that!